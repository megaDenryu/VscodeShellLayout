...
export interface 外殻レイアウトオプション {
    ...
    // Electronでtitlebarを除去したアプリ向け。trueにするとエディタタブバーの空き領域が
    // ウィンドウドラッグ領域(-webkit-app-region:drag)になる。タブ・閉じるボタン等の
    // 操作要素は常にno-dragなので操作は妨げない。省略時false（ブラウザでは無害に無視される）。
    タブバーをウィンドウドラッグ領域にする?: boolean;
    // オプトイン（省略時false=旧挙動）。trueのアプリだけ狭幅ビューポート(767px以下)で
    // アクティビティバーのテキストラベルを隠し、アイコンのみ表示にする（ラベルは
    // title属性のツールチップへ移す）。狭幅を想定しないJimbo/Pokemon等の既存アプリは
    // 未指定のままでよく、挙動は一切変わらない。
    狭幅ではラベルを省略する?: boolean;
}
