import type { テーマ配色 } from './テーマ型.ts';
import { 配色 } from './デフォルトテーマ.ts';
...
export const テーマCSS変数名: { readonly [K in keyof テーマ配色]: `--vsl-${string}` } = {
    ...
    ペインタブ非アクティブ背景: '--vsl-pane-tab-inactive-bg',
    ペインタブホバー背景: '--vsl-pane-tab-hover-bg',
    ペインタブ影: '--vsl-pane-tab-shadow',
