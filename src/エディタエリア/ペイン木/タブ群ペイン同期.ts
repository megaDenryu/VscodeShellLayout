// タブ群ペインの DOM 構築/差分同期。DOM同期.ts の ペイン木DOM同期 から呼ばれる。
...
//
// コンテンツ管理・タブ内ボタン管理へのアクセス手段(DOM同期コンテキスト)は
// コンストラクタで受け取り保持する。呼び出しのたびに引数で持参させない。
...
export class タブ群ペイン同期器 {
    constructor(private readonly _コンテキスト: DOM同期コンテキスト) {}

    同期する(新: タブ群ペイン, 旧: 構築済みタブ群 | null): 構築済みタブ群 {
        if (旧 === null) return this._新規構築(新);

        const 変化なし =
            新.タブ一覧 === 旧.ペイン.タブ一覧 &&
            新.選択中 === 旧.ペイン.選択中 &&
            新.タブ一覧.every(タブ => this._コンテキスト.タブ内ボタン取得(タブ.id) === 旧.タブボタン一覧記録.get(タブ.id));
        if (変化なし) return 旧;

        旧.タブバー.clearChildren();
        const { 要素一覧, ボタン一覧記録 } = this._タブ項目要素一覧を構築(新.タブ一覧, 新.選択中);
        旧.タブバー.childs(要素一覧);
        const 添付済みコンテンツ = this._コンテンツエリアを同期(
            旧.コンテンツエリア, 新.タブ一覧, 新.選択中, 旧.添付済みコンテンツ,
        );
    ...
    private _新規構築(新: タブ群ペイン): 構築済みタブ群 {
        const タブバー = div({ class: styles.タブバー })
            .setAttribute(タブバー属性, 新.id)
            .setAttributeIf({
                If: this._コンテキスト.タブバードラッグ領域,
                True: {
    ...
    private _コンテンツエリアを同期(
        ...
        for (const タブ of 新タブ一覧) {
            ...
            const コンテンツ = 既存アタッチ済み ?? this._コンテキスト.コンテンツ取得(タブ.id);
