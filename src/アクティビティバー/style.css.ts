...
// 常時非表示対応（オプトイン、外殻レイアウトオプション「アクティビティバーラベルを常に隠す」参照）:
// data-常時ラベル省略="true" を明示指定したアプリだけ、ビューポート幅に関わらずラベルを
// 隠す（VSCode本家と同じくアイコン+ツールチップのみ表示）。48px幅にラベル文字を収めようと
// すると日本語は任意の文字間で折り返され「エクスプロ/ーラー」のような読めない分断が起きるため、
// このオプションを選んだアプリはラベルの表示自体をやめる。属性未指定の既存アプリは無関係。
globalStyle(`${バー}[data-常時ラベル省略="true"] ${項目ラベル}`, {
    display: 'none',
});
